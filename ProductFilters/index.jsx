...
const ProductFilters = () => {
  ...
  const filteredProducts = (products) => {
    return products.filter((product) => flavor ? product.flavor === flavor : product).filter((product) => color ? product.color === color : product)
  ...
  return (
    <div className="container" data-testid="container">
      ...
      >
        ...
        {showLoadMore ? products.slice(0, 6).map((product, index) => <ProductTile product={product} id={index} key={index}/>): products.map((product, index) => <ProductTile product={product} id={index} key={index}/>)}
      ...
      {/*Once Load More Button is Clicked and All the Products are displayed on the UI, User should be able to see Load More Button*/}
      {showLoadMore && <div
